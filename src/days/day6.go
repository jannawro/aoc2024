...
func main() {
	...
	traversed := make(map[Position][]int)
...
func part1(grid [][]int, start Position, traversed map[Position][]int) int {
	var current, next Position
	direction := 0
	current = start
	for current.x > 0 && current.x < len(grid[0])-1 && current.y > 0 && current.y < len(grid)-1 {
		traversed[current] = append(traversed[current], direction)
		next.x = current.x + Directions[direction].x
		next.y = current.y + Directions[direction].y
...
func part2(grid [][]int, start Position, traversed map[Position][]int) int {
	var count int
	wg := sync.WaitGroup{}
...
func looped(p, start Position, grid [][]int) bool {
	...
	for current.x > 0 && current.x < len(gridVariant[0])-1 && current.y > 0 && current.y < len(gridVariant)-1 {
		if traversed[current] == nil {
			traversed[current] = []int{}
		}
		traversed[current] = append(traversed[current], direction)
		next = Position{
		...
		if gridVariant[next.y][next.x] == 0 {
			if slices.Contains(
			...
			current = next
		} else if gridVariant[next.y][next.x] == 1 {
			direction = (direction + 1) % 4
		}
	}
	gridVariant[p.y][p.x] = 0
	return false
}
